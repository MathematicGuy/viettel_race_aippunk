...
print('BEGIN SAVE DATA TO LOCAL/REMOTE MILVUS DB')
# Configuration
PATH = 'Public_test_input'
OUTPUT_PATH = 'Public_test_output'
IMAGE_BASE_DIR = f'{OUTPUT_PATH}/images'
MILVUS_URI = r"https://in03-7b3b56e59d62e9d.serverless.aws-eu-central-1.cloud.zilliz.com"
...
clip_model, clip_preprocess = clip.load("ViT-B/32", device=device)  # pip install git+https://github.com/openai/CLIP.git
clip_model.eval()
...
def save_to_milvus_pipeline(url, token, images_dir):
    ...
    print(f"\n{'='*60}")
    print("Initializing hybrid pipeline (ONCE for all files)...")
    print(f"{'='*60}")

    # ✅ CREATE PIPELINE ONCE - before the loop
    process_docs, search_engine = create_hybrid_pipeline(
        OUTPUT_PATH, IMAGE_BASE_DIR, MILVUS_URI, MILVUS_TOKEN, text_model, image_encoder
    )

    # ✅ REUSE pipeline for all file folders
    for file_name in file_names:
        ...
        # ✅ REUSE process_docs function (same Milvus collection)
